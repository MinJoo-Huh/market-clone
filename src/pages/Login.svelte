...
<script>
  ...
  const loginWithGoogle = async () => {
    try {
      const result = await signInWithPopup(auth, provider); // sign in, popup 결과 기다리기
      const credential = GoogleAuthProvider.credentialFromResult(result); // 인증 정보 가져오기
      const token = credential.accessToken; // 받아온 인증 정보 안에는 토큰과 유저의 정보가 있다.
...
<div>
  <!-- {#if $user$}
    <div>{$user$?.displayName}님 로그인 되셨습니다</div>
  {/if} -->
  <div>로그인하기</div>
